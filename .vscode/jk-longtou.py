# 克隆自聚宽文章：https://www.joinquant.com/post/29359
# 标题：市场情绪择时对追强势股的有效性分析
# 作者：智子投资
...
def initialize(context):
    ...
    g.max_list_yesterday_count = []   #全局变量需要在头部先定义
    g.max_zt_days_of_yesterday = 1     #全局变量需要在头部先定义
    g.total_is_run = 0
...
def before_trading_start(context):
    ...
    if len(g.buy_list):
        ...
        #if(len(g.max_list) >= len(g.max_list)[-1]):
    if len(g.buy_list) >= len(g.max_list_yesterday_count) or g.max_zt_days >= g.max_zt_days_of_yesterday:
        g.total_is_run=1
    else:
    ...
    g.max_list_yesterday_count = g.buy_list   #全局变量需要在头部先定义
    g.max_zt_days_of_yesterday = g.max_zt_days     #全局变量需要在头部先定义
...
# 如果你不是在聚宽平台上运行代码，你需要自己定义 SecurityNotExist 异常类
